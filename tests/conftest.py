...
def User_Table(DB_Connection):
    ...
    session.add_all([User(forename=f, surname=s, sex=sx)
        for (f,s,sx) in zip(firstnames,surnames, sex, strict=True)])
    session.commit()
...
def Alias_Table(DB_Connection):
    ...
    sex = ['Male', 'Male', 'Female', 'Male', 'Male',
            'Male', 'Male', 'Male', 'Male', 'Male']
    session.add_all([Alias(forename=f, surname=s, sex=sx)
        for (f,s,sx) in zip(afirstnames,asurnames,sex, strict=True)])
    session.commit()
